...
export function reportHtml(jobs, generatedAt) {
  ...
  <title>${jobs.length} new relevant job${jobs.length === 1 ? '' : 's'}</title>
  ...
  <body><div class="wrap"><header><h1>${jobs.length} new relevant job${jobs.length === 1 ? '' : 's'}</h1><p>Official career portals checked - ${escapeHtml(generatedAt)}</p></header>${cards}</div></body></html>`;
}

export function reportMarkdown(jobs, generatedAt, maxLength = Number.POSITIVE_INFINITY) {
  let report = `# ${jobs.length} new relevant job${jobs.length === 1 ? '' : 's'}\n\nOfficial career portals checked - ${escapeMarkdown(generatedAt)}\n`;
  let included = 0;
...
async function sendEmail(jobs, html, markdown, config) {
...
async function createGitHubIssue(jobs, markdown, config) {
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      title: `[Job Monitor] ${jobs.length} new relevant job${jobs.length === 1 ? '' : 's'}`,
      body: markdown,
    }),
...
export async function notify(jobs, config) {
  fs.mkdirSync(config.reportsDir, { recursive: true });
  const now = new Date();
  const stamp = now.toISOString().replace(/[:.]/g, '-');
  const generatedAt = now.toLocaleString('en-IN', { timeZone: 'Asia/Kolkata', dateStyle: 'medium', timeStyle: 'short' });
  const html = reportHtml(jobs, generatedAt);
  const markdown = reportMarkdown(jobs, generatedAt);
  const issueMarkdown = reportMarkdown(jobs, generatedAt, 60_000);
  const reportPath = path.join(config.reportsDir, `new-jobs-${stamp}.html`);
  const markdownPath = path.join(config.reportsDir, `new-jobs-${stamp}.md`);
  fs.writeFileSync(reportPath, html, 'utf8');
  fs.writeFileSync(markdownPath, markdown, 'utf8');
  fs.writeFileSync(path.join(config.reportsDir, 'latest.html'), html, 'utf8');
  ...
  if (emailConfigured) {
    try {
      await sendEmail(jobs, html, markdown, config);
  ...
  if (githubConfigured) {
    try {
      const issue = await createGitHubIssue(jobs, issueMarkdown, config);
